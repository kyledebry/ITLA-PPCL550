...
class ITLA:
    ...
    REG_FreqGHz=0x36 # Frequency, GHz*10 part. Sets the frequency to the right of the decimal point in units of 0.1 GHz. R/W
    ...
    REG_Cscanon=0xE5
    REG_Csweepon=0xE5
    REG_Csweepoffset=0xE6 # Returns offset of clean sweep in units of 0.1 GHz with offset of 200 GHz. Offset is (read-out - 2000) * 0.1 GHz. R
    ...
    def Send_command(self,byte0,byte1,byte2,byte3):
        self.sercon.write(chr(byte0))
        self.sercon.write(chr(byte1))
        self.sercon.write(chr(byte2))
        self.sercon.write(chr(byte3))
    ...
    def ITLACommunicate(self,register,data,rw):
        lock=threading.Lock()
        lock.acquire()
    ...
    def ITLACommunicateSR(self, register, data, rw):
        ...
        resp_unsigned = self.ITLACommunicate(register, data, rw)
    ...
    def ITLAFWUpgradeStart(self,raydata,salvage=0):
        ...
        if salvage==0:
            ref=ITLA.stripString(self.ITLACommunicate(ITLA.REG_Serial,0,0))
            if len(ref)<5:
            ...
            self.ITLACommunicate(ITLA.REG_Resena,0,1)
        self.ITLACommunicate(ITLA.REG_Iocap,64,1) #bits 4-7 are 0x04 for 115200 baudrate
        ...
        if ITLA.stripString(self.ITLACommunicate(ITLA.REG_Serial,0,0))<>ref:
            return(self.sercon,'After change baudrate: serial discrepancy found. Aborting. '+str(ITLA.stripString(self.ITLACommunicate(ITLA.REG_Serial,0,0))))
        #load the ray file
        self.raybin=raydata
    ...
    def ITLAFWUpgradeComplete(self):
        ...
        self.ITLACommunicate(ITLA.REG_Iocap,0,1) #bits 4-7 are 0x0 for 9600 baudrate
        self.sercon.close()
        #validate communication with the laser
        self.sercon = serial.Serial(self.tempport, 9600, timeout=1)
        ref=ITLA.stripString(self.ITLACommunicate(ITLA.REG_Serial,0,0))
        if len(ref)<5:
            return( self.sercon,'After change back to 9600 baudrate: serial discrepancy found. Aborting. '+str(ITLA.stripString(self.ITLACommunicate(ITLA.REG_Serial,0,0))))
        return(self.sercon,'')
